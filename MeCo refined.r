...
# Pleas feel free to give us feedbacks! 
...
# Set up the data 💾
    ...
    # and NF (Normal Fibroblasts) cultured on 3D_soft and 3D_stiff substrates.
        ...
        soft <- read_excel("-PATH-/3D_NF_so.xlsx")
        stiff <- read_excel("-PATH-/3D_NF_st.xlsx")

    # Combine the two expression matrix (soft & stiff) to have a single one with only the expression levels of CAFs 
    # We want to keep the ENSEMBLid for each gene for later use in DESeq2, so we keep the first column. 
    ...
    # DESeq2 Anlysis 🧬
        # Metadata and preparation of the data
          ...
          # This will enable DESeq2 to recongnise which sample is associated which condition.
          # This way we can compare the gene expression of CAF grown on 3D_soft substrate (control), 
          # with the gene expression of CAF gwon on 3D_stiff (treatment).
        ...
        # Performing DGE and taking care of FDR(False Discovery Rate) 🔬
          ...
          # For FDR we used the Yoav Benjamini and Yosef Hochberg methods, being the suited to our understanding. 
          ...
          # We store the results in a data frame and label the differentialy expressed genes with UP or DOWN, 
          # if the gene is up regulated or down regualed, respectively.
...
# Polishing and checking the data 🔎
    ...
    # Also load the phenotype dataset of the TCGA-KIRC project. 
        ...
        geno_data <- read.delim("-PATH-/TCGA-KIRC.htseq_fpkm.tsv")
        pheno_data <- read.delim("-PATH-/TCGA-KIRC.GDC_phenotype.tsv")
    ...
    # The genom_data and pheno_data datasets share information about 607 patients
    ...
    # Finally update the geno_data with only the patientid of primary tumoral samples, with a defined stage and keeping the Ensembl_id
...
# Given a list of genes, Metascape analyse in which patway each gene is involved and return the most influenced pathways. 
    # Metascape 📈
        # We need an excel file with all the differentialy expressed genes
            library("writexl")

            DifferentialExpressedGenes <- c(DOWN,UP) 
            write_xlsx(data.frame(DifferentialExpressedGenes), 'PathwayAnalysis2.xlsx')
    ...
    # Manually curated Pathways 🛣
        ...
        # We chose to use the 5 most rapresentative pathways in our sample of CAF and 
        ...
        # For each pathway subdived the genes in UP and DOWN regulated according to the
    ...
    # Computing the MeCo refined scores ⭐️⭐️⭐️
        # We call it refined beacouse it is pathway based, 
        # so it is more easily interpret and also more meaningful for assessing the role of 
        # mechanotranduction on tumoral cells. 
